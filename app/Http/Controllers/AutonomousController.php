<?php

namespace App\Http\Controllers;

use App\Models\Autonomous;
use App\Models\Query;
use App\Models\QueryValue;
use Illuminate\Http\Request;

class AutonomousController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index(Request $request)
    {
        $perPage = $request->input('per_page', 5);
        $search = $request->input('search');
        $searchColumn = $request->input('search_column', 'name'); // Padrão para 'name'

        $queries = Query::whereHas('autonomous', function ($query) use ($search, $searchColumn) {
            if ($search) {
                $query->where($searchColumn, 'like', "%{$search}%");
            }
        })->paginate($perPage);

        return view('autonomous.index', compact('queries', 'search', 'searchColumn'));
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        return view('autonomous.create');
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        // $request->validate(['cpf' => 'required|string|max:11']);
        $user = auth()->user();
        $queryValue = QueryValue::where('enterprise_id', $user->enterprise_id)->first();
        $plates = $request->vehicleCount;
        if ($plates) {
            $value = $queryValue->autonomous_base + ($queryValue->autonomous_additional * $plates);
        } else {
            $value = $queryValue->autonomous_base;
        }
        $query = new Query;
        $query->type = 'Autonomous';
        $query->status = "pending";
        $query->enterprise_id = $user->enterprise_id;
        $query->user_id = $user->id;
        $query->value = $value;
        $query->save();

        $autonomousData = $request->all();
        $autonomousData['query_id'] = $query->id;

        $autonomous = Autonomous::create($autonomousData);

        return redirect()->route('autonomous.index')->with('success', 'Consulta criada com sucesso!');
    }

    public function show($id)
    {
        $user = auth()->user();
        $query = Query::whereHas('autonomous')->where('id', $id)->first();
        $autonomous = $query->autonomous;
        return view('layouts.show_generic', [
            'entity' => $autonomous,
            'title' => 'Detalhes do Autônomo',
            'backRoute' => 'autonomous.index'
        ]);
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(Autonomous $autonomous)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, Autonomous $autonomous)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(Autonomous $autonomous)
    {
        //
    }
}
